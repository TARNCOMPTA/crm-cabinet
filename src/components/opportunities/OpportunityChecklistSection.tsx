...
import {
  loadChecklistsForOpportunity,
  createChecklistForOpportunity,
  createChecklistFromTemplateForOpportunity,
  deleteChecklist,
  addChecklistItem,
  updateChecklistItem,
  deleteChecklistItem,
...
export function OpportunityChecklistSection({ cardId, userId }: Props) {
  ...
  const refresh = useCallback(async () => {
    try {
      const data = await loadChecklistsForOpportunity(cardId);
      setChecklists(data);
    } catch {
  ...
  async function handleCreate() {
    ...
    if (!title) return;
    try {
      await createChecklistForOpportunity(userId, cardId, title);
      setNewTitle('');
  ...
  async function handleCreateFromTemplate(template: ChecklistTemplateWithItems) {
    try {
      await createChecklistFromTemplateForOpportunity(userId, cardId, template);
